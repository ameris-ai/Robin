...
#ifdef SH1106
        esp_err_t ret = esp_lcd_new_panel_sh1106(panel_io_, &panel_config, &panel_);
#else
        esp_err_t ret = esp_lcd_new_panel_ssd1306(panel_io_, &panel_config, &panel_);
#endif

        // 如果创建面板失败（比如没接屏幕），直接打印日志并跳出，不再继续
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "No display detected or failed to create panel object");
            return; 
        }

        // 以下操作如果失败也不要紧，不再使用 ESP_ERROR_CHECK
        esp_lcd_panel_reset(panel_);
        if (esp_lcd_panel_init(panel_) != ESP_OK) {
            ESP_LOGW(TAG, "Display init failed, continuing without screen");
            return;
        }
